...
const secretKey = "asdfg1234!@#$";
const bcryptSaltRounts = 10;
const jwtExpireDays = "2d";
async function createJwtToken(id) {
  return jwt.sign({ id }, secretKey, { expiresIn: jwtExpireDays });
}
...
export async function login(req, res, next) {
  ...
  if (!user) {
    res.status(404).json(`${userid}님 찾을 수 없음`);
  }
  const isValidPassword = await bcrypt.compare(password, user.password);
  if (!isValidPassword) {
    return res.status(404).json({ message: "아이디 또는 비밀번호 틀림" });
  }
  const token = await createJwtToken(user.id);
